 ...
 * </pre>
 * 
 * </blockquote> is equivalent to the three statements above, though for
 * repeated matches it is less efficient since it does not allow the compiled
 ...
 * Character.MIN_CODE_POINT} &nbsp;&lt;=&nbsp;<tt>0x</tt>
 * <i>h...h</i>&nbsp;&lt;=&nbsp; {@link java.lang.Character#MAX_CODE_POINT
 ...
 * <td headers="matches">An alphabetic character:{@code [\p Lower}\p{Upper}]}</td>
 * </tr>
 * <tr>
 ...
 * <td headers="matches">An alphanumeric character:{@code [\p Alpha}\p{Digit}]}</td>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td valign="bottom" headers="construct backref"><tt>\</tt>
 * <i>k</i>&lt;<i>name</i>&gt;</td>
 ...
 * <a href="#COMMENTS">x</a> <a href="#UNICODE_CHARACTER_CLASS">U</a> on - off</td>
 * </tr>
 * <tr>
 ...
 * </ul>
 * <p>
 * A <tt>named-capturing group</tt> is still numbered as described in <a
 * href="#gnumber">Group number</a>.
 * <p>
 ...
 * Compatibility Properties</i> of <a
 ...
 * <tr>
 * <td><tt>\p{Blank}</tt></td>
 * <td>A space or a tab: {@code [\p IsWhite_Space}
 * &&[^\p{gc=Zl}\p{gc=Zp}\x0a\x0b\x0c\x0d\x85]]}</td>
 * </tr>
 * <tr>
 * <td><tt>\p{Cntrl}</tt></td>
 ...
 * </table>
 * <p>
 * <a name="jcc"> Categories that behave like the java.lang.Character boolean
 ...
 * <p>
 * <tt>\X&nbsp;&nbsp;&nbsp;&nbsp;</tt>Match Unicode <a
 * href="http://www.unicode.org/reports/tr18/#Default_Grapheme_Clusters">
 * <i>extended grapheme cluster</i></a>
 * </p>
 * </li>
 * <li>
 * <p>
 * The backreference constructs, <tt>\g{</tt><i>n</i><tt>}</tt> for the
 * <i>n</i><sup>th</sup><a href="#cg">capturing group</a> and <tt>\g{</tt>
 * <i>name</i><tt>}</tt> for <a href="#groupname">named-capturing group</a>.
 * </p>
 * </li>
 * <li>
 * <p>
...
public final class Pattern implements java.io.Serializable {
	 ...
	 * the behavior of <tt>.</tt>, <tt>^</tt>, and <tt>$</tt>.
	 * <p>
	 ...
	 * Standard #18: Unicode Regular Expression</i></a> <i>Annex C:
	 * Compatibility Properties</i>.
	 * <p>
	...
	transient GroupRegistry registry;
	transient UnprocessedPattern unproc;
	/**
	 ...
	 * </pre>
	 * 
	 * </blockquote> behaves in exactly the same way as the expression
	 * <blockquote>
	 * 
	 * <pre>
	 ...
	 * <p>
	 * The input <tt>"boo:and:foo"</tt>, for example, yields the following
	 * results with these expressions: <blockquote>
	...
	private void readObject(java.io.ObjectInputStream s)
		...
		s.defaultReadObject();
		// Initialize counts
		unproc = new UnprocessedPattern(this::flags, registry);
		registry = new GroupRegistry();
		localCount = 0;
		// if length > 0, the Pattern is lazily compiled
		compiled = false;
		if (pattern.length() == 0) {
			unproc.root = new Start(lastAccept);
	...
	private void compile() {
		unproc.hasSupplementary = unproc.codepoints.compile(pattern);
		// Allocate all temporary objects here.
		unproc.buffer = new int[32];
		registry.clear();
		parse();
		// Release temporary storage
		unproc.codepoints.temp = null;
		unproc.buffer = null;
		unproc.codepoints.patternLength = 0;
		compiled = true;
	}
	private void parse() {
		if (has(LITERAL)) {
			// Literal pattern handling
			matchRoot = newSlice(unproc.codepoints.temp,
					unproc.codepoints.patternLength,
					unproc.hasSupplementary);
			matchRoot.next = lastAccept;
		} else {
			// Start recursive descent parsing
			matchRoot = expr(lastAccept);
			unproc.codepoints.confirmEnding();
		}
		// Peephole optimization
		if (matchRoot instanceof Slice) {
			unproc.root = BnM.optimize(matchRoot);
			if (unproc.root == matchRoot) {
				unproc.root = unproc.hasSupplementary ? new StartS(
						matchRoot) : new Start(matchRoot);
			}
		} else if (matchRoot instanceof Begin || matchRoot instanceof First) {
			unproc.root = matchRoot;
		} else {
			unproc.root = unproc.hasSupplementary ? new StartS(matchRoot)
					: new Start(matchRoot);
		}
	}
	/**
	...
	private boolean findSupplementary(int start, int end) {
		for (int i = start; i < end; i++) {
			if (isSupplementary(unproc.codepoints.temp[i])) return true;
		}
		return false;
	}
	/**
	 ...
	 */
	static final boolean isSupplementary(int ch) {
	...
	private Node expr(Node end) {
		...
		for (;;) {
			...
			Node nodeTail = unproc.root; // double return
			if (prev == null) {
				prev = node;
				firstTail = nodeTail;
			} else {
			...
			if (unproc.codepoints.peek() != '|') { return prev; }
			unproc.codepoints.next();
		}
	}
	...
	private Node sequence(Node end) {
		...
		LOOP: for (;;) {
			...
			switch (ch) {
				case '(':
					...
					continue;
				case '[':
					node = unproc.clazz(true);
					break;
				case '\\':
					ch = unproc.codepoints.nextEscaped();
					if (ch == 'p' || ch == 'P') {
						boolean oneLetter = true;
						boolean comp = (ch == 'P');
						ch = unproc.codepoints.next(); // Consume { if
													// present
						if (ch != '{') {
							unproc.codepoints.unread();
						} else {
							oneLetter = false;
						}
						node = unproc.family(oneLetter, comp);
					} else {
						unproc.codepoints.unread();
						node = atom();
					}
				...
				case '+':
					unproc.codepoints.next();
					throw unproc.codepoints
							.error("Dangling meta character '"
									+ ((char) ch) + "'");
				case 0:
					if (unproc.codepoints.cursor >= unproc.codepoints.patternLength) {
						break LOOP;
					}
					// Fall through
				default:
		...
		tail.next = end;
		unproc.root = tail; // double return
		return head;
	}
	@SuppressWarnings("fallthrough")
	/**
	...
	private Node atom() {
		...
		for (;;) {
			switch (ch) {
				...
				case '{':
					if (first > 1) {
						unproc.codepoints.cursor = prev; // Unwind one
													// character
						first--;
					}
					break;
				...
				case '\\':
					...
					if (ch == 'p' || ch == 'P') { // Property
						if (first > 0) { // Slice is waiting; handle it
							...
							boolean comp = (ch == 'P');
							boolean oneLetter = true;
							ch = unproc.codepoints.next(); // Consume {
														// if
							// present
							if (ch != '{')
								unproc.codepoints.unread();
							else oneLetter = false;
							return unproc.family(oneLetter, comp);
						}
					}
					unproc.codepoints.unread();
					prev = unproc.codepoints.cursor;
					ch = unproc.escape(false, first == 0, false);
					if (ch >= 0) {
						unproc.append(ch, first);
						first++;
						if (isSupplementary(ch)) {
							hasSupplementary = true;
						}
						ch = unproc.codepoints.peek();
						continue;
					} else if (first == 0) { return unproc.root; }
					// Unwind meta escape sequence
					unproc.codepoints.cursor = prev;
					break;
				case 0:
					if (unproc.codepoints.cursor >= unproc.codepoints.patternLength) {
						break;
				...
				default:
					...
					ch = unproc.codepoints.next();
					continue;
			}
			break;
		}
		if (first == 1) {
			return newSingle(unproc.buffer[0], flags);
		} else {
			return newSlice(unproc.buffer, first, hasSupplementary);
		}
	}
	// !!!<ref>
	// !!!<...>
	...
	private Node group0() {
		...
		int save = flags;
		unproc.root = null;
		int ch = unproc.codepoints.next();
		if (ch == '?') {
			ch = unproc.codepoints.skip();
			switch (ch) {
				case ':': // (?:xxx) pure group
				...
				case '<': // (?<xxx) look behind
					ch = unproc.codepoints.read();
					if (ASCII.isLower(ch) || ASCII.isUpper(ch)) {
						// named captured group
						String name = unproc.groupname(ch);
					...
					int start = unproc.codepoints.cursor;
					head = createGroup(true);
					tail = unproc.root;
					head.next = expr(tail);
					...
					head.study(info);
					if (info.maxValid == false) { throw unproc.codepoints
				...
				case '$':
				case '@':
					throw unproc.codepoints.error("Unknown group type");
		...
		unproc.codepoints.accept(')', "Unclosed group");
		flags = save;
		// Check for quantifiers
		Node node = unproc.closure(head);
		...
		if (node instanceof Ques) {
			...
			if (head.study(info)) { // Deterministic
				head = unproc.root = new GroupCurly(head.next, curly.cmin,
		...
		throw unproc.codepoints.error("Internal logic error");
	}
	/**
	...
	private Node createGroup(boolean anonymous) {
		...
		unproc.root = new GroupTail(localIndex, groupIndex);
		return head;
	}
	@SuppressWarnings("fallthrough")
	/**
	...
	//
	static final int countChars(CharSequence seq, int index,
	 ...
	 */
	static CharProperty newSingle(final int ch, final int flags) {
		if (has(flags, CASE_INSENSITIVE)) {
			int lower, upper;
			if (has(flags, UNICODE_CASE)) {
		...
		if (isSupplementary(ch)) return new SingleS(ch); // Match a given
												// Unicode character
	 ...
	 * the match() method.
	 */
	static boolean inRange(int lower, int ch, int upper) {
		return lower <= ch && ch <= upper;
	}
	/**
	 * Returns node for matching characters within an explicit value range.
	 */
	static CharProperty rangeFor(final int lower, final int upper) {
	 ...
	 */
	static boolean hasBaseCharacter(Matcher matcher, int i, CharSequence seq) {
	...
	public Stream<String> splitAsStream(final CharSequence input) {
		class MatcherIterator implements Iterator<String> {
			...
			public boolean hasNext() {
				...
				while (matcher.find()) {
					nextElement = input.subSequence(current,
							matcher.start()).toString();
					current = matcher.end();
					if (!nextElement.isEmpty()) {
						return true;
					} else if (current > 0) { // no empty leading substring
											// for zero-width
				...
				if (!nextElement.isEmpty()) {
					...
					// Ignore a terminal sequence of matching empty
					// elements
					emptyElementCount = 0;
					nextElement = null;
					return false;
